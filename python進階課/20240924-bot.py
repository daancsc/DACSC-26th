...
async def on_message(msg):
    ...
    if links:  # 如果訊息內容有連結
        ...
        links_str = '\n'.join(links)  # 將連結列表轉換為字串
        word = msg.content.replace(links_str, f'(一個網址, 網址標題是: "{str(title)}")' if title else '(一個網址, 網址無法辨識)')
        history = await update_history("使用者說: " + word)
